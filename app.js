...
var Player = function(param) {
    ...
    //creating objects for modifiers and powerups
    self.mod = {
        pwrs: {},
        spdMod: 1
    };
    ...
    self.updateSpd = function() {
        newSpd = self.baseSpd * self.mod.spdMod;
        if (self.pressingRight && self.pressingLeft)
            self.spdX = 0;
        else if (self.pressingRight && !self.checkForCollision(self.x + newSpd, self.y))
            self.spdX = newSpd;
        else if (self.pressingLeft && !self.checkForCollision(self.x - newSpd, self.y))
            self.spdX = -newSpd;
        else
            self.spdX = 0;
        ...
        if (self.pressingUp && self.pressingDown)
            self.spdY = 0;
        else if (self.pressingUp && !self.checkForCollision(self.x, self.y - newSpd))
            self.spdY = -newSpd;
        else if (self.pressingDown && !self.checkForCollision(self.x, self.y + newSpd))
            self.spdY = newSpd;
        else
            self.spdY = 0;
...
var Powerup = function(param) {
    ...
    self.w = (75 / 2);
    self.h = (75 / 2);
    self.pwrNum = Math.floor(Math.random() * 2);
    switch (self.pwrNum) {
        case 0:
            self.type = "bulletFrenzy";
            break;
        case 1:
            self.type = "1HitKill";
            break;
        case 2:
            self.type = "speedBurst";
            break;
    }

    if (self.type === "bulletFrenzy" || self.type === "1HitKill" || self.type === "speedBurst") {
        self.duration = 20;
        self.maxUses = 0;
    ...
    self.update = function() {
        //if has been picked up by player, it will
        if (self.pickedUp) {
            self.update.asMapObject();
        } else {
            self.update.asPlayerAttribute();
        }
    }

    self.update.asMapObject = function() {
        //if has existed longer than 20 seconds, removes itself
        if (time - self.timer >= 20)
            self.toRemove = true;
        ...
        for (var i in Player.list) {
            var p = Player.list[i];
            if (!p.isZombie) {
                //if collided, pass itself to player under pwrs[self.type], removed from Powerup.list
                if (self.x - self.w < p.x + PimgW && self.x + self.w > p.x - PimgW && self.y - self.h < p.y + PimgH && self.y + self.h > p.y - PimgH && Player.list[this.id] != Player.list[i]) {
                    console.log("pwr detected");
                    p.score += 2;
                    p.mod.pwrs[self.type] = this;
                    self.pickedUp = true;
                    self.parent = p.id;
                    self.timer = time;
                    self.toRemove = true;
    ...
    self.update.asPlayerAttribute = function() {
        //if has been active on player for more than duration, will delete itself from their pwrs
        if (time - self.timer >= self.duration && self.uses === 0)
            Player.list[self.parent].mod.pwrs[self.type] = {};
    }
