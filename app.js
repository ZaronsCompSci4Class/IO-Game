...
function Entity(param) {
	...
	this.checkForCollision = function(x, y) {
		...
		for (var i in Container.Players.list) {
			var p = Container.Players.list[i];
			if (x - PimgW < p.x + PimgW && x + PimgW > p.x - PimgW && y - PimgH < p.y + PimgH && y + PimgH > p.y - PimgH && Container.Players.list[this.id] != Container.Players.list[i]) {
...
function Player(param) {
	...
	Container.Players.list[this.id] = this;
...
Container.Players.onConnect = function(socket) {
	...
	socket.emit('init', {
		selfId: socket.id,
		player: Container.Players.getAllInitPack(),
		bullet: Bullet.getAllInitPack(),
		obj: Objective.getAllInitPack(),
		pwr: Powerup.getAllInitPack(),
	})
}
Container.Players.getAllInitPack = function() {
	var players = [];
	for (var i in Container.Players.list)
		players.push(Container.Players.list[i].getInitPack());
	return players;
}

Container.Players.onDisconnect = function(socket) {
	delete Container.Players.list[socket.id];
	removePack.player.push(socket.id);
}
Container.Players.update = function() {
	var pack = [];
	for (var i in Container.Players.list) {
		var player = Container.Players.list[i];
		player.update();
	...
	self.update = function() {
		...
		for (var i in Container.Players.list) {
			...
			if (self.map === p.map && self.getDistance(p) < 32 && self.parent !== p.id && p.isZombie) {
				...
				if (p.hp <= 0) {
					var shooter = Container.Players.list[self.parent];
					if (shooter)
						shooter.score += 1;
					p.hp = p.hpMax;
...
var Objective = function(param) {
	...
	self.update = function() {
		...
		for (var i in Container.Players.list) {
			var p = Container.Players.list[i];
			if (!p.isZombie) {
				if (self.x - self.w < p.x + PimgW && self.x + self.w > p.x - PimgW && self.y - self.h < p.y + PimgH && self.y + self.h > p.y - PimgH && Container.Players.list[this.id] != Container.Players.list[i]) {
	...
	this.updateAsMapObject = function() {
		...
		for (var i in Container.Players.list) {
			...
			if (!p.isZombie) {
				...
				if (this.x - this.w < p.x + PimgW && this.x + this.w > p.x - PimgW && this.y - this.h < p.y + PimgH && this.y + this.h > p.y - PimgH && Container.Players.list[this.id] != Container.Players.list[i]) {
					this.parent = p;
...
io.sockets.on('connection', function(socket) {
	...
	socket.on('signIn', function(data) {
		...
		Container.Players.onConnect(socket);
		pCounter++;
	...
	socket.on('disconnect', function() {
		...
		pCounter--;
		Container.Players.onDisconnect(socket);
	});
...
function pickZombie() {
	...
	Container.Players.list[zNum].isZombie = true;
}
...
setInterval(function() {
	...
	var pack = {
		player: Container.Players.update(),
